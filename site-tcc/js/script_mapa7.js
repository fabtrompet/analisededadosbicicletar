$(document).ready(function(){
      ...
      var dias_semana = new Array("Domingo", "Segunda-feira",
       ...
       "Sexta-feira", "Sábado");
      function get_dados2(id_estacao,dia_semana){
        return $.ajax({
          url: "http://localhost/site-tcc/dados.php",
          method: "POST",
          data: {tipo: "mediatemposem", id_estacao: id_estacao, dia_semana: dia_semana},
          dataType: 'json',
          success: function(data) {
            ...
            media = parseFloat(parseFloat(data[0].media).toFixed(2));
            
            error = parseFloat(data[0].error);

            margemmais = media + error;
            margemmenos = media - error;
            
            margemmais = parseFloat(margemmais.toFixed(2));
            margemmenos = parseFloat(margemmenos.toFixed(2));
      ...
      var ctx2 = [];
      var chartdata2 = [];
      function reload2(id_estacao,nome_estacao,dia_semana,maior){ 
        $.when(get_dados2(id_estacao,dia_semana)).done(function(){
          //if(chart)chart.destroy();
          console.log(dia_semana);
         var chart = new CanvasJS.Chart("chartdiv", {
        animationEnabled: true,
        title:{
          text: "Intervalo Médio em Horas de Indisponibilidade de Bicicletas ("+dias_semana[dia_semana]+")"
        },
        axisY:{
          title: "Horas",
          interval: 2 
        ...
        data: [{
          type: "column",
          name: "Horas",
          toolTipContent: "{label} <br> <b>{name}:</b> {y} horas",
          dataPoints: [
            { y: media, label: "Média" }
          ]
        },
        {
          ...
          toolTipContent: "<b>{name}:</b> {y[0]} - "+margemmenos+" horas",
          dataPoints: [
            { y: [margemmais, margemmenos]}
...
chart.render();
      ...
      function get_dados(id_estacao){
        return $.ajax({
          url: "http://localhost/site-tcc/dados.php",
          method: "POST",
          data: {tipo: "porcentagem_dia_semana_sem", id_estacao: id_estacao},
          dataType: 'json',
          success: function(data) {
            console.log(data);
            quantidade = [];
            ...
            for(var i in data) {
              ...
              dia_semana.push(dias_semana[data[i].dia_semana]);
            }
      ...
      //var ctx = [];
      var chartdata = [];
      function reload(id_estacao,nome_estacao){ 
        $.when(get_dados(id_estacao)).done(function(){
          //if(chart)chart.destroy();
          $("#myChart").remove();
          chartdata = [];
          $('#result').html('<canvas id="myChart" height="80px"></canvas>');
          chartdata = {
            labels: dia_semana,
            datasets : [
            {
              label: "Estação "+nome_estacao,
         ...
         enabled: false
          ...
          reload2(id_estacao,nome_estacao,0,100);
         $('#myChart').click(
            function(evt){
            ...
            if (firstPoint !== undefined)
                reload2(id_estacao,nome_estacao,dias_semana.indexOf(label),100);
            });
