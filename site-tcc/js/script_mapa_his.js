$(document).ready(function(){
    ...
    method: "POST",
    data: {tipo: "dadoshisto", temp: temp},
    dataType: 'json',
    success: function(data) {
      ...
      function getPoints(temp) {
        return $.when(update(temp)).done(function(){
        ...
        for (var i = 0; i < estacoes.length; i++){
          coordenadas[i] = {location: new google.maps.LatLng(estacoes[i].lng,estacoes[i].lat), weight: estacoes[i].quant}
        }
      ...
      window.initMap = function() {
        map = new google.maps.Map(document.getElementById('mapacalorhis'), {
          zoom: 13,
        ...
        //window.setTimeout(iniciar,10000)
        function iniciar(temp){
        $.when(getPoints(temp)).done(function(){

        heatmap.set('data', null);
        heatmap.setData(coordenadas);
        var hash = location.hash.replace(/[#\/]/g, '') || 'home';
        //if (hash == "mapacalorhis"){
        //window.setTimeout(iniciar,10000);
        //} 
        });
      }
      ...
      $("#data").datetimepicker({
          ...
          secondMin: 1,
          secondMax: 1,
          minDate: new Date('2017-04-01 00:00:01'),
