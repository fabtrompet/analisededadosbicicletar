$(document).ready(function(){
      ...
      var estacoes = [];
      function update(lat,lng,dia_semana,hora,raio){
        return $.ajax({
          url: "http://localhost/site-tcc/dados.php",
          method: "POST",
          data: {tipo: "melhoresestacoes", lat: lat, lng:lng, dia_semana:dia_semana, hora:hora, raio:raio},
          dataType: 'json',
          success: function(data) {
            ...
            for(var i in data) {
              ...
              estacoes.push({
                ...
                "lat": data[i].lat,
                "lng": data[i].lng,
                "porcentagem": parseFloat(parseFloat(data[i].total).toFixed(2)),
      ...
      function getPoints(lat,lng,dia_semana,hora,raio) {
        return $.when(update(lat,lng,dia_semana,hora,raio)).done(function(){
      ...
      window.initMap = function() {
        ...
        directionsDisplay.setMap(map);
         new google.maps.event.addListener(map, 'click', function( event ){
          ...
          location3 = new google.maps.LatLng(event.latLng.lat(),event.latLng.lng());
  ...
  $.when(getPoints($("#lat").val(),$("#lng").val(),$("#dia_semana").val(),$("#hora").val(),$("#raio").val())).done(function(){
          if (teste == true){
            setMapOnAll(null);
            teste = false
          }
          estacoes.forEach(function(estacao){
            ...
            marker.addListener('click', function() {
              ...
              ultimo = infowindow;
              //reload(estacao.id_estacao,estacao.nome_estacao);
            });


          });
          $("#table").empty();
          $("#table").append('<table class="mt-4 table table-bordered table-hover"><thead class="thead-light" class="text-justify"><tr><th>Estação</th><th>Endereço</th><th>Porcentagem de Bicicletas Disponíveis</th></tr></thead><tbody id="tableconteudo"></tbody></table>');
          var string = []
          for (var i = 0; i < estacoes.length; i++){
                string.push(estacoes[i].lat)
                string.push(estacoes[i].lng);
                $("#tableconteudo").append('<tr><td><button value="'+string+'" class="nomeestacao btn btn-primary">'+estacoes[i].nome_estacao+'</button></td><td>'+estacoes[i].endereco+'</td><td>'+estacoes[i].porcentagem+'%</td></tr>');
                string = [];
          }
          var location4 = new google.maps.LatLng(estacoes[0].lat,estacoes[0].lng);
           
           function calculateAndDisplayRoute(directionsService, directionsDisplay,location4) {
      ...
      calculateAndDisplayRoute(directionsService,directionsDisplay,location4);
          $(".nomeestacao").on('click',function(){
            var valor = $(this).val();
            valor = valor.split(",");
            location4 = new google.maps.LatLng(valor[0],valor[1]);
            calculateAndDisplayRoute(directionsService,directionsDisplay,location4);
            $('html, body').animate({scrollTop: '0px'}, 0);
          }); 
        });
