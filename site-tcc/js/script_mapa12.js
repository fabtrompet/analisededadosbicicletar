$(document).ready(function(){
      ...
      var estacaodadosmedia = [];
      var estacaodadostempo = [];
      function update2(temp, id_estacao){
      return $.ajax({
    url: "http://localhost/site-tcc/dados.php",
    method: "POST",
    data: {tipo: "dadoshisto2", temp: temp, id_estacao: id_estacao},
    dataType: 'json',
    success: function(data) {
      ...
      estacaodadostempo = [];
      for(var i in data) {
        estacaodadosmedia.push(data[i].media_bic);
        estacaodadostempo.push(data[i].tempo);
      }
    }});}

     

      function plotar(temp, id_estacao,estacao) {
        return $.when(update2(temp, id_estacao,estacao)).done(function(){
        ...
        var myLineChart = new Chart(ctx, {
            type: 'line',
            data: {
              labels: estacaodadostempo,
              datasets:[{
                data: estacaodadosmedia,
                label: estacao,
                borderColor: "#3e95cd",
                fill: false
              }]
            ...
            title: {
              display: true,
              text: 'Média de Bicicletas Disponiveis'
            },
            scales:{
              yAxes:[{
                scaleLabel:{
                  display: true,
                  labelString: 'Média de Bicicletas Disponiveis'
                }
              }]
            }
          }
      ...
      function seleciona(id, estacao){
          $("#estacao").val(estacao);
          $("#id_estacao").val(id);
          iniciar($("#data").val(),$("#id_estacao").val(),$("#estacao").val());
      }
      


      function iniciar(temp,id_estacao,estacao){
        plotar(temp,id_estacao,estacao);
        $('html, body').animate({scrollTop: $(document).height()}, 1000);  
      }
      
      // iniciar('2017-04-03 00:00:01');
      ...
      $("#data").datepicker({
          ...
          dateFormat: 'yy-mm-dd',
          minDate: new Date('2017-04-02'),
          maxDate: new Date('2017-07-01'),
          onClose: function(){
            iniciar($("#data").val(),$("#id_estacao").val(),$("#estacao").val());
          }
        });  
